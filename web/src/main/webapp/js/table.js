$(function() {
    ...
    function bindData(b) {
        var data = {
            'currentPage': page,
            rows: $("#Pagination").data('pagesize') || 10
        };
        if ($('#searchForm').size()) {
            var args = getParam($('#searchForm')) || {};
            data = $.extend(data, args);
        }
        var url = $('#table').attr('href');
    ...
    function initPagination(result) {
        ...
        // 创建分页
        $("#Pagination").pagination(result.data.page.totalPages, {
        ...
        if ($(".ml10.total-items").length !== 0) {
            $(".ml10.total-items").html('总条数: ' + result.data.page.totalRows);
        } else {
            $("#Pagination").after('<span class="ml10 total-items"> 总条数: ' + result.data.page.totalRows + '</span>');
        }
    }
